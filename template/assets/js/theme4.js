    ...
    var ST = {
                ...
                return this.each(function() {
                    ...
                    var createSticky = function() {
                        ...
                        function updatePaddingTop() {
                            $(".slider-content, .breadcrumb-area").css({"padding-top": targetHeight});
                        }
                        updatePaddingTop();
                        function stickyHeader() {
                            ...
                            if(isResizing) {
                                ...
                                if($window.scrollTop() >= 100) {
                                    HeaderTarget.addClass("body-bg");
                                    HeaderTarget.find(showHeaderFirst).addClass("body-bg header-border");
                                } else {
                                    HeaderTarget.removeClass("body-bg");
                                    HeaderTarget.find(showHeaderFirst).removeClass("body-bg header-border");
                                }
                                if($window.scrollTop() >= scrollHeader) {
                                    HeaderTarget.addClass("sticky");
                                    const cssValue = {"transform": "translateY(-" + targetHeight + "px)"};
                                    if(!isRefreshed) {
                                        cssValue.transition = "transform 0.5s ease,background-color 0.5s ease";
                                    }
                                    HeaderTarget.css(cssValue);
                                    isRefreshed = false;
                                } else {
                                    HeaderTarget.removeClass("sticky");
                                    HeaderTarget.css({"transform": "translateY(0px)","transition": "transform 0.5s ease,background-color 0.5s ease"});
                                }
                                // Scroll direction check
                                if($window.scrollTop() > lastScroll) {
                                    $(".sticky").removeClass("header-sticky"); // Scrolling down
                                    HeaderTarget.find(showHeaderFirst).css({"transform": "translateY(0px)","transition": "transform 0.5s ease,background-color 0.5s ease"});
                                ...
                                if(lastScroll <= scrollHeader) {
                                    ...
                                    HeaderTarget.find(showHeaderFirst).removeClass("body-bg");
        ...
        shopContent: function() {
            ...
            $sortBy.on("change", function() {
                ...
                $selectWrapper.removeClass("secondary-color extra-bg").addClass("body-primary-color");
                $selectWrap.find("li:has([data-value='" + $selectedOption + "'])").addClass("selected");
                $selectWrap.find("li:has([data-value='" + $selectedOption + "']) a").addClass("secondary-color extra-bg").removeClass("body-primary-color");
            ...
            $document.on("click", "button.shop-view-btn", function() {
                ...
                $shopViewBtn.removeClass("primary-color disabled").addClass("body-color");
                $(this).addClass("primary-color disabled").removeClass("body-color");                
        ...
        productSwipe: function () {
            var productSwipeInit = () => {
                var $productSwiper = $(".product-swiper");
                if($productSwiper.length === 0) return;
                var $productSwipe = window.matchMedia("(min-width: 992px)"),
                    $productSwiperWrapper = $productSwiper.find(".product-swiper-wrapper"),
                ...
                // Use event delegation for tab clicks (delegated on static parent)
                $document.off("click.productTabs").on("click.productTabs", "#product-tab-nav .product-swiper-slide", function(e) {
                    e.preventDefault();
                    var $productSwiperSlideTab = $(this),
                        $activeTab = $productSwiperSlideTab.find("a").attr("href");
                    $productTabSwiperSlide.removeClass("active");
                    $productSwiperSlideTab.addClass("active");
                    $productSwiperTab.hide();
                    $($activeTab).show();
                });
                toggleSlider();
                $productSwipe.addEventListener("change", toggleSlider); // Modern method
            };
            productSwipeInit();
        },
        productJs: function() {
        ...
        contactJs: function() {
            function sectionOverlay() {
                var $sectionOverlay = $(".section-overlay");
                $sectionOverlay.css("margin-top", "");
                if(window.matchMedia("(min-width: 768px)").matches) {
                    $sectionOverlay.each(function() {
            ...
            var swiper = new Swiper(".swiper#contact-locationslider", {
                ...
                on: {
                    init: sectionOverlay,
                    slideChange: sectionOverlay,
                    resize: sectionOverlay
                }
            });
            $window.on("resize", sectionOverlay);
            // contact-banner-wrap js
            function contactBannerPosition() {
                const $wraps = $(".contact-banner-wrap");
                $wraps.css("margin-left", "");
                if (window.matchMedia("(min-width: 992px)").matches) {
                    const htmlOffset = $("html").offset().left;
                    $wraps.each(function () {
                        const offsetFromHtml = $(this).offset().left - htmlOffset;
                        $(this).css({
                            "margin-left": -offsetFromHtml + "px",
                            "visibility": "visible"
                        });
                    });
                }
            ...
            $window.on("resize", contactBannerPosition);
        },
        cartPage: function() {
        ...
        comingSoon: function() {
            ...
            // Initialize: hide all images, show the first one
        ...
        videoJs: function() {
            ...
            // handle click for video-btn-tag
            $document.on("click", ".video-btn-tag", function() {
                var $videoTag = $(this).closest(".video").find(".video-tag"),
                    $video = $videoTag.find("video"),
                    $videoGet = $video.get(0),
                    $videoBtn = $(this);
                if($videoGet.paused) {
                    $videoGet.play();
    ...
    ST.mainSlider = {
        init: function() {
            $("#home-slider").each(function() {
                ...
                $(".swiper-dots .swiper-pagination-homeslider .swiper-pagination-bullet")[0].innerHTML = '<span class="inner"></span><span class="bar left-bar"><span class="progress-wrap"></span></span><span class="bar right-bar"><span class="progress-wrap"></span></span>';
                homeslider.on("slideChange", function() {
                    homeslider.autoplay.stop();
                    homeslider.autoplay.start();
                    var buttons = document.querySelectorAll(".swiper-dots .swiper-pagination-homeslider .swiper-pagination-bullet");
                    buttons.forEach(function(button) {
                        if(button.classList.contains("swiper-pagination-bullet-active")) {
                            button.innerHTML = '<span class="inner"></span><span class="bar left-bar"><span class="progress-wrap"></span></span><span class="bar right-bar"><span class="progress-wrap"></span></span>';
                        } else {
                            button.innerHTML = '<span class="outer"></span>';
                        }
                    });
                });
            });
